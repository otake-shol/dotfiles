...
- セキュリティは `permissions` の allow/deny/ask で確保
...
| 設定 | 値 | 用途 |
|------|-----|------|
| Apple ID | `sholchamnyori.ax9@icloud.com` | EAS Submit / App Store Connect |
| Apple Team ID | `CM9325X2HW` | EAS Build / Submit |
| Google Play サービスアカウントキー | `./google-play-service-account.json`（プロジェクトルート配置） | EAS Submit |

※ ASC App ID（App Store Connect のアプリ固有ID）はアプリごとに異なる

### App Store スクリーンショットサイズ

| デバイス | サイズ（縦） | サイズ（横） | 用途 |
|---------|------------|------------|------|
| iPhone 6.5インチ | 1242 × 2688 | 2688 × 1242 | iPhone XS Max, 11 Pro Max 等 |
| iPhone 6.7インチ | 1284 × 2778 | 2778 × 1284 | iPhone 14 Pro Max, 15 Pro Max 等 |
