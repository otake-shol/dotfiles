...
現在ローカル MCP サーバーは未設定（`~/.claude.json` に mcpServers なし）。
接続済みなのは claude.ai 側コネクタ（Gmail / Google Calendar / Google Drive）のみ。

過去に使っていたサーバー（再導入する場合は `claude mcp add` で追加）:

| サーバー | 用途 |
|---------|------|
| Context7 | ドキュメント参照 |
| Playwright | ブラウザ自動化・E2Eテスト |
| GitHub | Issue/PR操作（`gh` CLI で代替可） |
| gws | Gmail/Drive/Calendar等（claude.aiコネクタで代替中） |
| hourei | e-Gov法令検索・条文取得 |
| tax-law | 税法・通達・裁決事例 |
| mf-ca | マネーフォワード クラウド会計（リモートMCP・OAuth） |

※ MCP サーバー定義は `~/.claude.json`（dotfiles 管理外）にあるため、再導入したら追加コマンドをこのファイルに記録すること。
